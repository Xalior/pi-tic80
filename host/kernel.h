//
// kernel.h — the Circle kernel that hosts TIC-80.
//
// Device ownership: this kernel brings up interrupts, the timer, a serial
// console carrying stdio, the SD card (FatFs, holding the cartridges and the
// configuration), the USB host controller and the cooperative scheduler.
// Video and audio belong to circle-libsdl2 and are created inside SDL_Init
// when the program calls it. So do the CPU clock and the case fan, which the
...
// else, before the split arms and before the program's first instruction. The
...
//   core 1   the APPLICATION core. TIC-80, alone: the studio loop, the
//            cartridge's script and nothing else. Every platform call it
...
private:
    ...
    // program opens — reaches the card through it without anything above
