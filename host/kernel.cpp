//
// kernel.cpp — brings the board up, then hands control to TIC-80.
//
// TIC-80 is an ordinary command-line program: it expects a working standard
// library, a filesystem holding its cartridges and its configuration, and an
// SDL2 implementation. This file supplies the first two and starts the third,
// then calls the program's entry point with a fixed argument list.
//
// EVERYTHING THIS PROGRAM TOUCHES LIVES IN ONE DIRECTORY ON THE CARD,
...
//
// Four things point at it, and between them they cover every path the program
// can reach:
//
//   argv[0]           an absolute path inside it.
//   --fs=<path>       upstream's own switch for the directory the console
//                     and the surf browser read cartridges from. TIC-80
//                     takes it in preference to the folder it derives for
//                     itself, so this is what puts the cartridges staged by
//                     `make card` in front of the user with nothing else
//                     configured.
//   the base path     SDL2Circle_DeclareBasePath, below. TIC-80 asks
//                     SDL_GetPrefPath where it may write, and that answer is
//                     composed under the base this kernel declares. It is
//                     where the configuration cartridge is written.
//   chdir             this kernel makes it the working directory before the
//                     program starts, so anything opened by a relative name
//                     lands there too rather than in the root.
//
...
// TIC-80's entry point. It is main() in the upstream source; the build
...
extern "C" int tic80_main(int argc, char **argv);

void CGlueStdioInit(CConsole &rConsole);

static const char From[] = "tic80";

// The program's command line.
//
// argv[0] is an absolute path rather than a bare program name, so a program
// that derives anything from where it was started from lands inside this
// game's own directory.
//
// --fs points TIC-80's own filesystem root at that directory. Without it the
// console starts in the directory SDL_GetPrefPath answers with, which is
// where the configuration goes and not where the cartridges are staged.
//
...
// this without the card or the build changing.
static const char *Tic80Argv[] = {
    RAPI_GAME_DIR "/tic80",
    "--fs=" RAPI_GAME_DIR,
};
...
// terminating null.
static const char *s_FinalArgv[sizeof(Tic80Argv) / sizeof(Tic80Argv[0])
...
void CSplitCores::Run(unsigned nCore)
{
    ...
    case 1:
        ...
        s_AppResult = tic80_main(s_FinalArgc, const_cast<char **>(s_FinalArgv));
...
boolean CKernel::Initialize(void)
{
    ...
    // is plausible. TIC-80 stamps saved cartridges with it.
...
{
    m_Logger.Write(From, LogNotice, "starting TIC-80");
    ...
    // The VIRTUAL display the program is given, declared before anything asks
    // the library about the display. Every SDL answer TIC-80 gets — the
    ...
    //
    // 256x144 is TIC80_FULLWIDTH by TIC80_FULLHEIGHT: the console's 240x136
    // screen plus the border it draws around it, which is the surface TIC-80
    // uploads whole every frame. It is a constant because it is a fact about
    // the program, not a setting — and because the size TIC-80 asks the
    // display for is what its own scale calculation reads back, a larger
    // declaration would only make it open a window it then letterboxes.
    //
    // The library has no default and no fallback: without this it refuses to
    // start.
    static const int VIRTUAL_WIDTH  = 256;
    static const int VIRTUAL_HEIGHT = 144;
    ...
    // Where SDL says this program lives, and therefore where SDL_GetPrefPath
    // composes the directory TIC-80 writes its configuration into. Declared
    // before SDL_Init, which is the library's requirement, and set to this
    // game's own directory for the reason every other path here is.
    if (SDL2Circle_DeclareBasePath(RAPI_GAME_DIR) != 0)
        m_Logger.Write(From, LogWarning, "base path " RAPI_GAME_DIR
                       " refused: %s", SDL_GetError());
    ...
    // Build the program's argument list before anything else looks at it: the
    // baked arguments, plus whatever a pre-boot writer stamped into the
    // image. Kernel switches are taken out here and never reach the program.
    s_FinalArgc = DefaultsBuildArgv(Tic80Argv,
                                    sizeof(Tic80Argv) / sizeof(Tic80Argv[0]),
                                    s_FinalArgv,
                                    sizeof(s_FinalArgv) / sizeof(s_FinalArgv[0]));

    // Move into this game's own directory before the program runs, so every
    ...
    // A failure is worth saying out loud but is not fatal — the program will
    ...
    // the two --rapi-cache switches have had their say. Before the program
    // runs, so the allocations it ever makes happen while the heap is still
    // empty, and every read the program makes meets a cache that is already
    // there.
    ...
    m_Logger.Write(From, LogNotice, "TIC-80 exited with %d — parked", res);
    for (;;)
    {
